...
void leer(int Ma[][3], int fi,int co){
...
int suma(int Ma[][3], int fi,int co){
	int s=0;
	for(int i=0; i<fi;i++){
		for(int j=0; j<co;j++){
			s=s+Ma[i][j];
...
float promedio (int Ma[][3], int fi,int co){
	float supro;
	supro=(float) suma(Ma,fi,co)/(fi*co);
	return (supro);
}

int mayor(int Ma[][3], int fi,int co){
	int may=Ma[0][0];
	for(int i=0; i<fi;i++){
		for(int j=0; j<co;j++){
			if (Ma[i][j]>may){
...
int main(int argc, char *argv[]) {
    ...
    cout<<"La suma de la diagonal principal es: "<<sumaDiagPrinc(matriz, fi, col)<<endl;
	system("PAUSE");
	return 0;
}
